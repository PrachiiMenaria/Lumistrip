...
import { Download, Sparkles, Image as ImageIcon, Type, Palette, ChevronLeft, RefreshCw, X, Plus, Minus } from "lucide-react";
...
const STICKER_CATEGORIES = [
  { name: "My PNG Stickers", items: ["/stickers/heart.svg", "/stickers/star.svg", "/stickers/sparkle.svg"] },
];
...
export default function EditingStudio() {
  ...
  const [stickerCategories, setStickerCategories] = useState(STICKER_CATEGORIES);

  useEffect(() => {
    const savedPhotos = sessionStorage.getItem("lumistrip_photos");
    const savedLayout = sessionStorage.getItem("lumistrip_layout");
    
    if (savedPhotos) setPhotos(JSON.parse(savedPhotos));
    else setPhotos(["/camera.png", "/camera.png", "/camera.png"]);
    
    if (savedLayout) setLayoutId(parseInt(savedLayout));
    setIsLoaded(true);
  ...
  const downloadImage = async () => {
    ...
    try {
      ...
      link.download = `lumistrip-${Date.now()}.jpg`;
      link.href = dataUrl;
      link.click();
    } catch (error) {
  ...
  return (
    <main className="relative w-full h-screen flex flex-col md:flex-row overflow-hidden bg-[#FFF8F2]">
      ...
      <section className="relative flex-1 h-full flex items-center justify-center p-4 md:p-8 overflow-y-auto z-10 transition-colors duration-500">
        <Link href="/capture" className="absolute top-8 left-8 flex items-center gap-2 px-4 py-2 bg-white/50 backdrop-blur-md rounded-full text-[#49344F] font-serif italic shadow-sm hover:bg-white transition-all z-20">
        ...
        >
          {photos.slice(0, layoutId).map((photo, i) => (
            <div key={i} className="relative w-full aspect-[4/3] bg-gray-100 overflow-hidden shadow-inner">
              <img src={photo} alt={`Photo ${i+1}`} className="w-full h-full object-cover scale-x-[-1]" />
            </div>
          ))}
      ...
      <section className="relative w-full md:w-[400px] flex-1 md:flex-none md:h-full bg-white/80 backdrop-blur-xl border-l border-white/20 shadow-[-10px_0_30px_rgba(0,0,0,0.05)] flex flex-col z-20 overflow-hidden">
        <div className="p-4 md:p-8 pb-4 shrink-0">
          <h1 className="font-serif italic text-3xl text-[#49344F] tracking-tight">Editing Studio</h1>
          <p className="font-sans text-xs uppercase tracking-widest text-[#49344F]/50 mt-2">Decorate your memories</p>
        </div>
        ...
        <div className="p-6 bg-white/50 border-t border-black/5 shrink-0">
          <motion.button onClick={downloadImage} whileHover={{ scale: 1.02, y: -2 }} whileTap={{ scale: 0.98 }} className="w-full py-5 bg-[#49344F] text-[#FFF8F2] rounded-2xl font-serif italic text-xl tracking-wide flex items-center justify-center gap-3 shadow-[0_10px_20px_rgba(73,52,79,0.2)] hover:shadow-[0_15px_30px_rgba(73,52,79,0.3)] transition-shadow">
